...
hexo.extend.helper.register('aside_categories', function (categories, options,flag) {
    ...
    if (t > 0) {
      prepareQuery(parent).forEach((cat, i) => {
        if (t > 0) {
          ...
          if (showCount) {
            result += `<span class="card-category-list-count">${options.brackets?"（":""}${cat.length}${options.brackets?"）":""}</span>`
          }
  ...
  var str='<ul class="card-category-list" id="aside-cat-list">'+list[0]+'</ul>'
  if(!flag){
    str=`<div class="item-headline">
